...
# v0.12 - 2018/04/24 - Add *TYPE command for machines that need it
#                      Fix debug message in OSGBPB()
#                      Update open file PTR for OSGBPB
#                      OSGBPB 8 ("read dir") wasn't limiting to the matches
#                         Thanks to dominicbeesley for these
# v0.13 - 2019/10/10 - Add the concept of folders with *HFOLDER *HCF *HMKF
# v0.14 - 2020/09/12 - CLOSE# was always closing all files
#                      *DIR wasn't allowing "*DIR 0"
#                       (turns out handling was bad for "*DIR UU" and similar)
#                      Mask out lower bits of OSFIND function call since
#                      we don't care about them
#                      Fix *TYPE handling of control characters (offbyone)
#                         Thanks to dominicbeesley for these
# v0.15 - 2020/11/08 - Add *DDIR as an alias for *HCF to match the new
#                        MMFS2 command.
#                      Allow *DCAT to have a filename to limit results.
#                        e.g. *DCAT *goril*
# v0.16 - 2021/06/08 - Fix "*.0"; previously it reported the current drive
#                      Send an error on OSFILE 255 if file not exist
...
sub get_disks()
{
  ...
  # that in that slot
  foreach (<$disk_base/$current_folder/[0-9]*.*>)
  {
    next unless -d $_;
    next if /^$disk_base\/$current_folder\/_/;  # Hide subdirectories
    s/^.*\///;
    my ($num,$name)=(/^(\d+)\.(.*)$/);
  ...
  $dsk++;
  # Now any other disk
  foreach (sort <$disk_base/$current_folder/*>)
  {
    next unless -d $_;
    next if /^$disk_base\/$current_folder\/_/;  # Hide subdirectories
    next if /\/_Library$/;
    ...
    $d{$dsk}=$name;
    debug(0,"$_ means disk $dsk name $name");
    $dsk++;
  }
  return(%d);
}
...
sub get_cat($;$)
{
  ...
  debug(30,"Looking in $disk_base/$disks{$dr}");
...
##    send_byte(0);
  ...
  elsif (cmd_match($cmd,"DCAT"))
  {
    debug(1,"*DCAT");
    my $want=$args[0] || '*';
    # Convert BBC wildcards to regex
    $want=~ s/(\W)/\\$1/g; # as per perlre
    $want=~s/\\#/./g;
    $want=~s/\\\*/.*/g;
    debug(20,"Regex is $want");
  ...
  elsif (cmd_match($cmd,"DCREATE"))
  {
    ...
    mkdir("$disk_base/$current_folder/$want");
    debug(10,"Made $disk_base/$current_folder/$want");
...
##    send_byte(0);
  ...
  elsif (cmd_match($cmd,'DIR'))
  {
    ...
    my ($dr,$di,$dname)=decompose(canonify("$d.dummy"));
    error(206,"Bad dir") unless $dname eq "DUMMY";
    ($drive,$dir)=($dr,$di);
  ...
  elsif (cmd_match($cmd,"HFOLDERS"))
  {
    my $r="Folders present:\r\n  ..\r\n";
    foreach (<$disk_base/$current_folder/_*>)
    {
      s/^$disk_base\/$current_folder\///;
      $r .= "  $_\r\n";
    }
    send_output($r);
  }
  elsif (cmd_match($cmd,"HCF") || cmd_match($cmd,"DDIR"))
  {
    my $d=$args[0];
    ...
    if ($d ne "")
    {
      if ($d =~ /\//)
      {
        error(253,"Bad characters present");
      }
      elsif ($d eq "^")
      {
        $current_folder="";
      ...
      {
        error(253,"Bad filename: missing _");
      }
      elsif ( -d "$disk_base/$current_folder/$d" )
      {
        $current_folder .= "/$d";
        $current_folder =~ s!^/!!;
      }
    ...
    send_output("Current folder is: $current_folder\r\n");
  }
  elsif (cmd_match($cmd,"HMKF"))
  {
    my $d=need_arg($args[0]);
    ...
    debug(10,"Creating $disk_base/$current_folder/$d");
    mkdir("$disk_base/$current_folder/$d") || do
    {
      debug(1,"Failed to create $disk_base/$current_folder/$d");
      error(201,"Can not create");
    };
    send_output("Created");
  }
...
sub OSFILE()
{
  ...
  # if it doesn't
  # On LOAD return an error, otherwise send back response code 0
  if ($cmd == 255 && !$cat{$key})
  {
    error(214,"Not found");
  }
  elsif ($cmd !=0 && $cmd != 7 && !$cat{$key})
  {
...
sub OSFIND()
{
  ...
  # Mask out unwanted bits in function code
  $fn = $fn & 0xf0;

  # function 0 (close) takes a byte; everything else takes a file
  my $parm=$fn?extract_filename(read_string):read_byte;
...
while(1)
{
  debug(0,"Character received from client: " . read_char(1));
}
